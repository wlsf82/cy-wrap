describe('cy.wrap', () => {
  ...
  it('spreads the values of an array', () => {
    cy.wrap([1, 'a', false]).spread((first, second, third) => {
  ...
  it('works with the yielded array', () => {
    cy.wrap([1, 'a', false]).then(theArray => {
      expect(theArray).to.deep.equal([1, 'a', false])
    })
  })
  ...
  it('works with the yielded obj', () => {
    ...
    cy.wrap(obj).then(theObject => {
      expect(theObject).to.deep.equal({
        name: 'Ross Geller',
        series: 'Friends'
      })
    })
